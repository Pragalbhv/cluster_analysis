...
- Facet areas are computed using `CoordinationPolyhedraModifier` and read from
  the surface mesh: `data.surfaces['coord-polyhedra'].faces['Area']`.
...
from ovito.modifiers import VoronoiAnalysisModifier, CoordinationPolyhedraModifier
from ovito.pipeline import Pipeline, PythonSource
from ovito.data import Particles, SimulationCell, SurfaceMesh
...
def _compute_voronoi(
    ...
    # Add CoordinationPolyhedraModifier to create polyhedra surface mesh with face areas
    poly = CoordinationPolyhedraModifier()
    pipeline.modifiers.append(poly)
...
def _extract_bond_pairs_and_areas(data) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # Face areas are stored in CoordinationPolyhedraModifier's surface mesh
    areas = np.zeros(len(pairs), dtype=float)
    
    # DEBUG: Check what's available in data
    print(f"DEBUG: Has surfaces attribute: {hasattr(data, 'surfaces')}")
    if hasattr(data, 'surfaces'):
        print(f"DEBUG: Available surface identifiers: {list(data.surfaces.keys())}")
    
    # Try to get face areas from coordination polyhedra surface mesh
    surface_mesh = data.surfaces.get('coord-polyhedra', None)
    if surface_mesh is not None:
        print(f"DEBUG: Found coord-polyhedra surface mesh")
        faces = surface_mesh.faces
        print(f"DEBUG: Number of faces: {faces.count}")
        print(f"DEBUG: Available face properties: {list(faces.keys())}")
        
        if "Area" in faces and "Adjacent Cell" in faces:
            face_areas = np.asarray(faces["Area"].array, dtype=float)
            adjacent_cells = np.asarray(faces["Adjacent Cell"].array, dtype=int)
            
            print(f"DEBUG: Face areas shape: {face_areas.shape}")
            print(f"DEBUG: Face areas non-zero count: {np.count_nonzero(face_areas)}")
            print(f"DEBUG: Sample face areas (first 10): {face_areas[:10]}")
            print(f"DEBUG: Adjacent cells shape: {adjacent_cells.shape}")
            print(f"DEBUG: Adjacent cells ndim: {adjacent_cells.ndim}")
            
            # Map face areas to bond pairs using adjacent cells
            # Adjacent cells are in a Nx2 array where each row is [particle_i, particle_j]
            if adjacent_cells.ndim == 2 and adjacent_cells.shape[1] == 2:
                # Build lookup: (min(i,j), max(i,j)) -> area
                for face_idx in range(len(face_areas)):
                    cell0, cell1 = adjacent_cells[face_idx]
                    area = face_areas[face_idx]
                    # Find matching bond pair (order-independent)
                    bond_idx = None
                    for bidx in range(len(pairs)):
                        if (pairs[bidx][0] == cell0 and pairs[bidx][1] == cell1) or \
                           (pairs[bidx][0] == cell1 and pairs[bidx][1] == cell0):
                            bond_idx = bidx
                            break
                    if bond_idx is not None:
                        areas[bond_idx] = area
            else:
                print(f"DEBUG: Adjacent cells not in expected format (Nx2), got ndim={adjacent_cells.ndim}")
        else:
            print(f"DEBUG: Missing Area or Adjacent Cell property in faces")
            print(f"DEBUG: 'Area' in faces: {'Area' in faces}")
            print(f"DEBUG: 'Adjacent Cell' in faces: {'Adjacent Cell' in faces}")
    else:
        print(f"DEBUG: No coord-polyhedra surface mesh found")

    print(f"DEBUG: Final areas non-zero count: {np.count_nonzero(areas)}")
    return pairs, areas
...
def build_voronoi_graph_from_pipeline(
    ...
    try:
        ...
        try:
            pipeline.modifiers.remove(poly)
        except Exception:
            pass
        try:
